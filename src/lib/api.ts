...
const PLAGIARISM_API_URL =
  "https://veritext-backend-cfnq.onrender.com/api/plagiarism-check";
...
export async function checkPlagiarism(text: string): Promise<PlagiarismResult> {
  ...
  // ✅ FIX: backend returns "results", not "matches"
  ...
  const sentenceCount =
    text
      .split(/(?<=[.!?])\s+/)
      .filter((s) => s.trim().length > 10).length || 1;
  ...
  return {
    ...
    doc1Lines:
      doc1Lines.length > 0
        ? doc1Lines
        : [
            "The study examines the impact of technology on education.",
            "Traditional methods are being replaced by digital tools.",
            "Students show improved engagement with interactive content.",
            "However, screen time concerns remain significant.",
    ...
    doc2Lines:
      doc2Lines.length > 0
        ? doc2Lines
        : [
            "This study analyzes the effect of technology on modern education.",
            "Traditional methods are being replaced by digital tools.",
            "Students show improved engagement with interactive content.",
            "A new methodology for measuring engagement was introduced.",
            "Screen time concerns remain but are mitigated by structured usage.",
            "More longitudinal studies are recommended.",
          ],
  };
}

const GEMINI_API_KEY =
  "AIzaSyDED9J1FMqhP1kHsXyxx0iytAm-rDn_aO4";
...
${sentences.join("\n")}`;
  ...
  try {
    ...
    return {
      ...
      sentences: (parsed.sentences || []).map(
        (s: { text: string; probability: number }) => ({
          text: s.text,
